...
void execute(enum IpcCommand command, char *request) {
	...
	log_debug("========%s request=================\n%s\n----------------------------------------", ipc_command_name(command), request);
	...
	for (;;) {
		...
		log_debug("========%s response================\n%s\n----------------------------------------", ipc_command_name(command), response);
...
main(int argc, char **argv) {
	...
	if (strcmp(argv[1], ipc_command_name(GET)) == 0) {
		fn = get;
	} else if (strcmp(argv[1], ipc_command_name(CFG_WRITE)) == 0) {
		fn = cfg_write;
	} else if (strcmp(argv[1], ipc_command_name(CFG_SET)) == 0) {
		fn = cfg_set;
	} else if (strcmp(argv[1], ipc_command_name(CFG_DEL)) == 0) {
		fn = cfg_del;
	} else {
		example_usage();
	}
