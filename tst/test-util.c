...
static int auto_scale_setup(void **state) {
	...
	s->head->desired_mode = calloc(1, sizeof(struct Mode));
...
static void auto_scale_missing(void **state) {
	...
	// null desired_mode
	struct Mode *mode = s->head->desired_mode;
	s->head->desired_mode = NULL;
	scale = auto_scale(s->head);
	assert_int_equal(scale, wl_fixed_from_int(0));
	s->head->desired_mode = mode;
...
static void auto_scale_valid(void **state) {
	...
	s->head->desired_mode->width = 2560;
	s->head->desired_mode->height = 1440;
	...
	s->head->desired_mode->width = 3840;
	s->head->desired_mode->height = 2160;
	scale = auto_scale(s->head);
