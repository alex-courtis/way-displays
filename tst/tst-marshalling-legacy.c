// original test before migration, testing libyaml
...
void unmarshal_cfg_from_file__ok(void **state) {
	...
	struct Cfg *read = yaml_unmarshal_file("tst/marshalling-legacy/cfg-all.yaml", yaml_root_to_cfg);
...
void unmarshal_cfg_from_file__empty(void **state) {
	...
	struct Cfg *read = yaml_unmarshal_file("tst/marshalling-legacy/cfg-empty.yaml", yaml_root_to_cfg);

	assert_nul(read);

	assert_log(ERROR, "\nparsing file tst/marshalling-legacy/cfg-empty.yaml no root node\n");
...
void unmarshal_cfg_from_file__bad(void **state) {
	...
	struct Cfg *read = yaml_unmarshal_file("tst/marshalling-legacy/cfg-bad.yaml", yaml_root_to_cfg);
...
void unmarshal_cfg_from_file__legacy(void **state) {
	struct Cfg *read = yaml_unmarshal_file("tst/marshalling-legacy/cfg-legacy.yaml", yaml_root_to_cfg);
	cfg_apply_defaults(read);
...
void marshal_cfg__ok(void **state) {
	...
	char *actual = yaml_marshal(cfg_actual, yaml_doc_cfg, "cfg");
...
void marshal_ipc_response__map(void **state) {
	...
	char *expected = read_file("tst/marshalling-legacy/ipc-responses-map.yaml");
...
void unmarshal_ipc_responses__map(void **state) {
	char *yaml = read_file("tst/marshalling-legacy/ipc-responses-map.yaml");
...
void unmarshal_ipc_responses__seq(void **state) {
	char *yaml = read_file("tst/marshalling-legacy/ipc-responses-seq.yaml");
