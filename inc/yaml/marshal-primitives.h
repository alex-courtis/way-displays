...
bool yaml_map_add_pset (struct MC *c, const char *key, const struct PSet*  const pset,  fn_yaml_node_from_v fn,  int mapping);
bool yaml_map_add_list (struct MC *c, const char *key, const struct SList* const list,  fn_yaml_node_from_v fn,  int mapping);
bool yaml_map_add_smap (struct MC *c, const char *key, const struct SMap*  const smap,  fn_yaml_node_from_kv fn, int mapping);
bool yaml_map_add_smapi(struct MC *c, const char *key, const struct SMapI* const smapi, fn_yaml_node_from_ki fn, int mapping);
