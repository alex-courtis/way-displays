...
void desire_scale(struct Head *head) {
	...
	// user scale first
	const struct SImapFilter f = { .key_data = (fn_pred_sp)head_name_desc_matches_head, .data = head, };
...
void desire_adaptive_sync(struct Head *head) {
	...
	struct SsetFilter f = { .val_data = (fn_pred_sp)head_name_desc_matches_head, .data = head, };
...
const struct Plist *desire_order(const struct Sset * const order_name_desc, const struct PPmap * const heads) {
	...
	fill_order_buckets(buckets, candidates, (fn_pred_pp)head_matches_name_desc_regex);
