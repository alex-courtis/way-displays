...
static bool seq_add_str(const void *data, int sequence) {
	if (!data || !sequence)
		return false;
...
static bool seq_add_user_scale(const void *data, int sequence) {
	if (!data || !sequence)
		return false;
...
static bool seq_add_user_mode(const void *data, int sequence) {
	if (!data || !sequence)
		return false;
...
static bool seq_add_user_transform(const void *data, int sequence) {
	if (!data || !sequence)
		return false;
...
static bool seq_add_condition(const void *data, int sequence) {
	if (!data || !sequence)
		return false;
	...
	if (condition->plugged)
		map_key_to_list("PLUGGED", condition->plugged, seq_add_str, map);

	if (condition->unplugged)
		map_key_to_list("UNPLUGGED", condition->unplugged, seq_add_str, map);

	return yaml_document_append_sequence_item(ctx.document, sequence, map);
}

static bool seq_add_disabled(const void *data, int sequence) {
	if (!data || !sequence)
		return false;
	...
	if (disabled->conditions) {
		...
		return map &&
			...
			map_key_to_list("IF", disabled->conditions, seq_add_condition, map) &&
			yaml_document_append_sequence_item(ctx.document, sequence, map);
	} else {
		return seq_add_str(disabled->name_desc, sequence);
	}
}
...
static bool map_cfg(const void *data, int mapping) {
	...
	map_key_to_list (cfg_element_name(ORDER),                 cfg->order_name_desc,             seq_add_str,            mapping);
	...
	map_key_to_list (cfg_element_name(SCALE),                 cfg->user_scales,                 seq_add_user_scale,     mapping);
	map_key_to_list (cfg_element_name(MODE),                  cfg->user_modes,                  seq_add_user_mode,      mapping);
	map_key_to_list (cfg_element_name(TRANSFORM),             cfg->user_transforms,             seq_add_user_transform, mapping);
	map_key_to_list (cfg_element_name(VRR_OFF),               cfg->adaptive_sync_off_name_desc, seq_add_str,            mapping);
	...
	map_key_to_list (cfg_element_name(DISABLED),              cfg->disabled,                    seq_add_disabled,       mapping);
...
static bool seq_add_mode(const void *data, int sequence) {
	if (!data)
		return false;
...
static bool seq_add_head(const void *data, int sequence) {
	if (!data || !sequence)
		return false;
	...
	map_key_to_list("MODES", head->modes, seq_add_mode, map);
...
static bool map_state(const void *data, int mapping) {
	...
	map_key_to_list("HEADS", heads, seq_add_head, mapping);

	return true;
}

static bool seq_add_log_cap_line(struct LogCapLine *line, int sequence) {
	int map = yaml_document_add_mapping(ctx.document, NULL, YAML_BLOCK_MAPPING_STYLE);
	if (!map)
		return false;
...
static bool map_messages(struct IpcOperation *operation, int mapping) {
	...
	for (struct SList *i = operation->log_cap_lines; i; i = i->nex) {
		...
		lines_added = seq_add_log_cap_line(cap_line, seq_lines) || lines_added;
...
static bool map_ipc_operation(struct IpcOperation *operation, int mapping) {
	if (!operation)
		return false;
...
char *marshal_ipc_response_2(struct IpcOperation *operation) {
	...
	if (!map_ipc_operation(operation, root))
		goto end;
